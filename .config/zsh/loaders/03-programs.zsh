...
#   2) lbin'!' means symlink instead of hard link ( which currently uses -P )
#   3) completions starting with _ are automatically inferred, those that don't need are directly mv'ed or downloaded
#   4) Items that are not based or a release or have a common ice pick structure are seperated to their own area ( on the same level - 0c)
zt 0c light-mode binary from'gh-r' lman lbin'!' for \
    bpick'*darwin*' mv'*gst*->gst' uetchy/gst

# Removed: old x64 versions.
