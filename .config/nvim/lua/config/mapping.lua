...
g.maplocalleader = '\\'
exec([[
    nmap <expr> <space> mapleader
    vmap <expr> <space> mapleader
...
-- Helps with identifying highlights under cursor
map('n', '<c-H>h', ':echo synIDattr(synID(line(\'.\'), col(\'.\'), 0), \'name\')<cr>')
map('n', '<c-H>t', ':TSHighlightCapturesUnderCursor<cr>')
